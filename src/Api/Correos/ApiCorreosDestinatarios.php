...
function crearDestinatario($enlace, $data)
{
    ...
    // Verificar si el email ya existe (solo si está ACTIVO)
    // Permite reutilizar emails de destinatarios eliminados (inactivos)
    $sqlCheck = "SELECT id FROM correos_destinatarios WHERE email = ? AND activo = 1";
...
function actualizarDestinatario($enlace, $data)
{
    ...
    // Verificar si el nuevo email ya existe en otro destinatario ACTIVO (excluyendo el actual)
    if (!empty($email)) {
        $sqlCheckEmail = "SELECT id FROM correos_destinatarios WHERE email = ? AND id != ? AND activo = 1";
        ...
        if ($stmtCheckEmail->num_rows > 0) {
            echo json_encode(["success" => false, "message" => "El email ya existe en otro destinatario activo"]);
...
function eliminarDestinatario($enlace, $data)
{
    ...
    // Eliminar completamente de la tabla
    $sql = "DELETE FROM correos_destinatarios WHERE id = ?";
    $stmt = $enlace->prepare($sql);
    $stmt->bind_param("i", $id);
