...
// Validar que el campo de fecha sea seguro
$camposPermitidos = ['fechaSalida', 'fechaEnroute', 'fechaDelivery', 'fechaIngreso'];
if (!in_array($campoFecha, $camposPermitidos)) {
    die(json_encode(["error" => "Campo de fecha no válido."]));
}
...
WHERE enc.{$campoFechaBD} BETWEEN ? AND ?
...
// ======================
// CLASE PDF PARA EL NUEVO REPORTE (SE MANTIENE IGUAL)
// ======================
class PDFDespachos extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, utf8_decode('PRODUCCION POR DÍA'), 0, 1, 'C');
...
foreach ($datosPorFecha as $fecha => $datosFecha) {
    ...
    // Agregar productos de esta fecha (ya consolidados)
...
// Generar PDF
$nombreArchivo = 'Produccion_' . date('Y-m-d_His') . '.pdf'; // 👈 Nuevo nombre para evitar cache
