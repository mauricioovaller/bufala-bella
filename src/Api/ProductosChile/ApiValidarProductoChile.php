<?php
//src/Api/ProductosChile/ApiValidarProductoChile.php
...
if ($tipo === "nuevo") {
    $sql = "SELECT Codigo_Siesa FROM ProductosChile WHERE Codigo_Siesa = ?";
    $stmt = $enlace->prepare($sql);
    $stmt->bind_param("s", $codigoSiesa);
} else {
    $sql = "SELECT Codigo_Siesa FROM ProductosChile WHERE Codigo_Siesa = ? AND Id_Producto <> ?";
    $stmt = $enlace->prepare($sql);
