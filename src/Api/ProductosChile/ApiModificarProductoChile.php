<?php
//src/Api/ProductosChile/ApiModificarProductoChile.php
...
try {
    $sql = "UPDATE ProductosChile SET
                DescripProducto = ?, DescripFactura = ?, Codigo_Siesa = ?, Codigo_FDA = ?, Codigo_CUST = ?,
                PesoGr = ?, PesoNetoUndGr = ?, FactorPesoBruto = ?, PrecioVenta = ?,
                PlanVallejo = ?, CodigoCIP = ?, DescripPlanVallejo = ?, FOB_Valor = ?, VAN_Valor = ?,
                DiasVencimiento = ?, Activo = ?
            WHERE Id_Producto = ?";
    $stmt = $enlace->prepare($sql);
    $stmt->bind_param("sssssddddissddiii",
        ...
        $diasVencimiento, $activo, $idProducto
    );
    $stmt->execute();
    $stmt->close();

    echo json_encode(["success" => true, "message" => "Producto Chile actualizado exitosamente"]);
