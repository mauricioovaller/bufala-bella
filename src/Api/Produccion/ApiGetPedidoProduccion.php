...
try {
    if ($tipo === 'chile') {
        // ========== CHILE ==========
        $sqlEnc = "SELECT Id_EncabPedido, Id_Cliente, PurchaseOrder, FechaOrden FROM EncabPedidoChile WHERE Id_EncabPedido = ?";
        $stmtEnc = $enlace->prepare($sqlEnc);
        $stmtEnc->bind_param("i", $idPedido);
        $stmtEnc->execute();
        $stmtEnc->bind_result($idEnc, $idCliente, $purchaseOrder, $fechaOrden);
        if (!$stmtEnc->fetch()) {
            echo json_encode(["success" => false, "message" => "Pedido Chile no encontrado"]);
            $stmtEnc->close(); $enlace->close(); exit;
        }
        $stmtEnc->close();

        $sqlCli = "SELECT Nombre FROM ClientesChile WHERE Id_Cliente = ?";
        ...
        $sqlDet = "SELECT d.Id_DetPedido, d.Id_Producto, p.DescripProducto, d.Cantidad,
                          d.Lote1, l.CodigoLote AS LoteCodigo,
                          d.FechaElaboracion, d.FechaVencimiento, p.DiasVencimiento,
        ...
        $stmtDet->execute();
        $stmtDet->bind_result($idDet, $idProducto, $descProducto, $cantidad, $lote1, $loteCodigo, $fechaElab, $fechaVenc, $diasVenc,
            $tempIni, $tempFin, $horaIni, $horaFin);
        ...
        while ($stmtDet->fetch()) {
            $items[] = [
                ...
                'cantidad' => $cantidad,
                'lote' => $lote1 ? ['id' => $lote1, 'codigo' => $loteCodigo] : null,
                'fechaElaboracion' => $fechaElab ?? '',
                'fechaVencimiento' => $fechaVenc ?? '',
                'diasVencimiento' => $diasVenc ?? 0,
        ...
        echo json_encode([
            ...
            "pedido" => [
                "idPedido" => $idPedido,
                "tipo" => "chile",
                "numero" => 'CHI-' . str_pad($idPedido, 6, '0', STR_PAD_LEFT),
                "idCliente" => $idCliente,
        ...
        $sqlDet = "SELECT d.Id_DetPedido, d.Id_Producto, p.DescripProducto, d.Cantidad,
                          d.Id_Responsable, r.Nombre AS ResponsableNombre,
                          d.Lote1, d.Lote2, d.Lote3,
                          l1.CodigoLote AS Lote1Codigo, l2.CodigoLote AS Lote2Codigo, l3.CodigoLote AS Lote3Codigo,
                          d.CantidadLote1, d.CantidadLote2, d.CantidadLote3
                   FROM $tablaDet d
        ...
        $stmtDet->bind_param("i", $idPedido);
        $stmtDet->execute();
        $stmtDet->bind_result($idDet, $idProducto, $descProducto, $cantidad,
            $idResponsable, $responsableNombre,
            $lote1, $lote2, $lote3, $lote1Codigo, $lote2Codigo, $lote3Codigo,
            $cantidadLote1, $cantidadLote2, $cantidadLote3);
        ...
        while ($stmtDet->fetch()) {
            $items[] = [
                ...
                'cantidades' => [intval($cantidadLote1), intval($cantidadLote2), intval($cantidadLote3)]
            ];
        }
        $stmtDet->close();
