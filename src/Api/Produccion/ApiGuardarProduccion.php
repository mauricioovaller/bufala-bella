...
$tipo = $data['tipo']; // 'normal', 'sample' o 'chile'
...
try {
    if ($tipo === 'chile') {
        // ========== CHILE: Lote1 + Fechas + Certificado Térmico ==========
        $sqlUpdate = "UPDATE DetPedidoChile SET 
            Lote1 = ?,
            FechaElaboracion = ?,
            FechaVencimiento = ?,
            TemperaturaInicial = ?,
        ...
        foreach ($items as $item) {
            ...
            $lote = isset($item['lote']) ? intval($item['lote']) : null;
            $fechaElab = $item['fechaElaboracion'] ?? null;
            $fechaVenc = $item['fechaVencimiento'] ?? null;
            $tempIni = isset($item['temperaturaInicial']) ? floatval($item['temperaturaInicial']) : null;
            $tempFin = isset($item['temperaturaFinal']) ? floatval($item['temperaturaFinal']) : null;
            $horaIni = $item['horaInicialPH'] ?? null;
            $horaFin = $item['horaFinalPH'] ?? null;
            
            $stmt->bind_param("issddssi", $lote, $fechaElab, $fechaVenc, $tempIni, $tempFin, $horaIni, $horaFin, $idDet);
            if (!$stmt->execute()) {
                throw new Exception("Error actualizando ítem Chile $idDet: " . $stmt->error);
            }
        }
    } else {
        // ========== NORMAL / SAMPLE ==========
        ...
        // Obtener cantidades disponibles para validación
        ...
        foreach ($items as $item) {
            ...
            $stmt->bind_param("iiiiiiiii", $idResponsable, $lote1, $lote2, $lote3, $cant1, $cant2, $cant3, $idUsuario, $idDet);
            if (!$stmt->execute()) {
