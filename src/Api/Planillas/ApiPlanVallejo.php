...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->SetX(90);
        $this->Cell(60, 4, 'Resolucion No. 18764072143394', 0, 1, 'C');
        $this->SetX(90);
        $this->Cell(60, 4, 'Vigente de Jun-04-2024 Hasta Dic-04-2025', 0, 1, 'C');
...
$pdf->Cell(0, 10, 'CUADRO INSUMO PRODUCTO', 0, 1, 'C');
$pdf->Ln(5);

// INFORMACIÓN DE FACTURA
$pdf->SetFont('Helvetica', 'B', 12);
$pdf->Cell(0, 6, 'FACTURA No. ' . $numero_factura, 0, 1, 'L');
$pdf->SetFont('Helvetica', 'B', 10);
$pdf->Cell(0, 6, 'CODIGO MP: MP3488', 0, 1, 'L');
$pdf->SetFont('Helvetica', '', 10);
$pdf->MultiCell(0, 5, 'MP IMPORTADA: MATERIAL DE EMPAQUE PARA EXPORTACIÓN DE QUESO MOZZARELLA MOZZARELLA BUFFALO BURRATA - CILIEGINE INFUSION RICOTTA MOZZARELLA BUFFALO ORGANIC ( CILIEGINE - BOCCONCINI OVOLINE)');
$pdf->Ln(2);

$pdf->SetFont('Helvetica', 'B', 10);
$pdf->Cell(0, 6, 'UNIDAD COMERCIAL: BASE Y TAPA', 0, 1, 'L');
$pdf->Ln(5);
...
$pdf->Cell(25, 8, 'LINEA EN FACTURA', 1, 1, 'C');

$pdf->SetFont('Helvetica', '', 9);
foreach ($detalles as $detalle) {
    $pdf->Cell(20, 6, $detalle['codigo_cip'], 1, 0, 'C');
    $pdf->Cell(80, 6, substr(utf8_decode($detalle['producto']), 0, 40), 1, 0, 'L');
    $pdf->Cell(20, 6, number_format($detalle['unidades'], 0), 1, 0, 'R');
    $pdf->Cell(25, 6, number_format($detalle['kilogramos'], 2), 1, 0, 'R');
    $pdf->Cell(25, 6, $detalle['item'], 1, 1, 'C');
}

// TOTALES
$pdf->SetFont('Helvetica', 'B', 9);
$pdf->Cell(100, 6, 'TOTAL', 1, 0, 'R');
...
$pdf->SetFont('Helvetica', 'B', 10);
$pdf->Cell(0, 6, 'POR FAVOR ELABORAR DOCUMENTO APARTE SIN PLAN VALLEJO', 0, 1, 'L');
$pdf->Ln(2);

$pdf->SetFont('Helvetica', '', 10);
$pdf->Cell(0, 6, number_format($kilos_netos_sin_plan_vallejo, 2) . ' KILOS NETOS  ' . number_format($kilos_brutos_sin_plan_vallejo, 2) . ' KILOS BRUTOS', 0, 1, 'L');
$pdf->Ln(2);

$pdf->SetFont('Helvetica', 'B', 10);
$pdf->Cell(0, 6, 'PARA EL DOCUMENTO DE SIN PLAN VALLEJO COLOCAR PIEZAS (BT) ' . $piezas_bt, 0, 1, 'L');
$pdf->Ln(5);

$pdf->SetFont('Helvetica', 'B', 10);
$pdf->Cell(0, 6, 'PLACA VEHICULO', 0, 1, 'L');
$pdf->SetFont('Helvetica', '', 10);
$pdf->Cell(0, 6, $placa ?: 'NO ASIGNADA', 0, 1, 'L');
