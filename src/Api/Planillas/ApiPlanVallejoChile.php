...
// CONSULTA 1: ENCABEZADO DESDE EncabInvoiceChile
...
$stmtEncabezado->bind_result($id_factura, $numero_factura, $cantidad_estibas, $tot_kgm_netos_factura, $placa);
...
// CONSULTA 2: DETALLE DESDE DetInvoiceChile + ProductosChile
$sqlDetalle = "SELECT
    det.Item,
    prd.CodigoCIP,
...
$stmtDetalle->execute();
$stmtDetalle->bind_result($item, $codigo_cip, $producto, $unidades, $kilogramos, $cajas);

$detalles = [];
$total_unidades_plan_vallejo = 0;
$total_kilos_plan_vallejo = 0;
while ($stmtDetalle->fetch()) {
    $detalles[] = ['item'=>$item,'codigo_cip'=>$codigo_cip,'producto'=>$producto,'unidades'=>$unidades,'kilogramos'=>$kilogramos,'cajas'=>$cajas];
...
foreach ($detalles as $d) {
    $pdf->Cell(20, 6, $d['codigo_cip'], 1, 0, 'C');
    $pdf->Cell(100, 6, utf8_decode($d['producto']), 1, 0, 'L');
    $pdf->Cell(20, 6, number_format($d['unidades'], 0), 1, 0, 'R');
    $pdf->Cell(25, 6, number_format($d['kilogramos'], 2), 1, 0, 'R');
    $pdf->Cell(30, 6, $d['item'], 1, 1, 'C');
}
...
$pdf->Output('I', 'Plan_Vallejo_Chile_' . $numero_factura . '.pdf');
?>
