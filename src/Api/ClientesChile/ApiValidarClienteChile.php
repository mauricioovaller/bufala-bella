<?php
//src/Api/ClientesChile/ApiValidarClienteChile.php
...
if ($tipo === "nuevo") {
    $sql = "SELECT Nombre FROM ClientesChile WHERE Nombre = ?";
    $stmt = $enlace->prepare($sql);
    $stmt->bind_param("s", $nombre);
} else {
    $sql = "SELECT Nombre FROM ClientesChile WHERE Nombre = ? AND Id_Cliente <> ?";
    $stmt = $enlace->prepare($sql);
