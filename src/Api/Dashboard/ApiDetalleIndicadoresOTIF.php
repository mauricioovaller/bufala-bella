...
try {
    if ($tipo === "inFull") {
        ...
        $sql = "SELECT
                ...
                LEFT JOIN ClientesRegion clr ON e.Id_ClienteRegion = clr.Id_ClienteRegion
                ...
                WHERE e.FechaSalida BETWEEN ? AND ?
                  ...
                  AND (
                    ...
                    OR e.Id_EncabPedido IN (
                        ...
                        FROM DetNotaCredito dnc2
                        INNER JOIN EncabNotaCredito encNC2 ON dnc2.Id_EncabNotaCredito = encNC2.Id_EncabNotaCredito
        ...
        $stmt->execute();
        $stmt->bind_result($idPedido, $po, $fechaSalida, $factura, $cliente, $region, $idDet, $idProd, $producto, $cantDesp, $cantPed, $cantCred);
        ...
        $sql = "SELECT
                ...
                GROUP BY e.Id_EncabPedido, e.PurchaseOrder, e.FechaSalida, e.FechaSalida_Orig, e.FacturaNo, c.Nombre, clr.Region
                ORDER BY e.Id_EncabPedido";

        $stmt = $enlace->prepare($sql);
        $stmt->bind_param("ss", $fechaDesde, $fechaHasta);
        $stmt->execute();
        $stmt->bind_result($idPedido, $po, $fechaReal, $fechaOrig, $factura, $cliente, $region, $totalProd, $totalCajas);
