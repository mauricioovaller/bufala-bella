<?php
//src/Api/PedidosChile/ApiImprimirListaEmpaquePrecios.php
...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(138, 5, 'SIC - SISTEMA INTEGRADO DE CALIDAD', 'LTR', 0,  'C');
        $this->Cell(34, 5, 'EXPORTACIONES', 'LTR', 1, 'C');

        $this->Cell(75, 5, '', 'LR', 0,  'C');
        $this->Cell(138, 5, '', 'LRB', 0,  'C');
        $this->Cell(19, 5, u8('Página'), 1, 0, 'C');
        $this->Cell(15, 5, u8('Código'), 1, 1, 'C');

        $this->Cell(75, 5, '', 'LR', 0,  'C');
        $this->Cell(138, 5, '', 'LRT', 0,  'C');
        $this->Cell(19, 5, '1 de 1', 1, 0, 'C');
        $this->Cell(15, 5, 'R-EXP-002', 1, 1, 'C');

        $this->Cell(75, 5, '', 'LR', 0,  'C');
        $this->Cell(138, 5, u8('SOLICITUD DE PRODUCCIÓN / LISTA DE EMPAQUE PRODUCTO EXPORTACIÓN CHILE'), 'LR', 0,  'C');
...
$pdf->SetMargins(5, 10, 5);                // Izquierda, arriba, derecha
$pdf->AliasNbPages();
$pdf->AddPage();

// ENCABEZADO DEL PEDIDO (ancho total = 247mm)
$anchoTotalEnc = 247;
...
{
    $ancho_etiqueta = 35;
    $ancho_texto = 247 - 35 - 36; // 176
    $ancho_oc = 36;
    $altura_linea = 6;
    $x_inicial = $pdf->GetX();
    $y_inicial = $pdf->GetY();
    $texto_utf8 = u8($texto);
    $ancho_texto_calculado = $ancho_texto - 2;
...
$anchos = [12, 10, 36, 30, 10, 9, 13, 13, 10, 10, 10, 13, 12, 12, 12, 12, 11, 14];
$idxRef = 2;
$idxPres = 3;
...
foreach ($detalles as $detalle) {
    ...
    $vals = [
        ...
        number_format($detalle['precio_unitario'], 4),
...
$pdf->Cell(12+10+36+30+10+9+13+13, 6, u8('TOTALES:'), 1, 0, 'L');  // cols 0-8
...
$pdf->Cell(13, 6, '', 1, 0, 'R');   // col 12 Peso Esc X Und
$pdf->Cell(12, 6, '', 1, 0, 'R');   // col 13 Peso X Caja
$pdf->Cell(12, 6, number_format($totalPesoNeto, 2), 1, 0, 'R'); // col 14 Kg Escurr
$pdf->Cell(12, 6, number_format($totalPesoNetoTotal, 2), 1, 0, 'R'); // col 15 Kg Escurr
$pdf->Cell(12, 6, number_format($totalPesoBruto, 2), 1, 0, 'R'); // col 13 Kg Brutos
$pdf->Cell(11, 6, '', 1, 0, 'R');   // col 14 Valor Unitario
$pdf->Cell(14, 6, number_format($totalValor, 2), 1, 0, 'R'); // col 15 Valor Total
$pdf->Ln();
...
if (!empty($observaciones)) {
    ...
    $pdf->SetFont('Arial', '', 9);
    $pdf->MultiCell(190, 6, u8($observaciones), 0, 1);
}
...
$pdf->Cell(247 - $anchoBloqueTotal, $altoFilaTotal, '', 0, 0);
