<?php
//src/Api/PedidosSample/ApiImprimirMultiplesBOL.php
...
// ======================
// CLASE PDF PARA MÚLTIPLES BOLs DE SAMPLES
// ======================
class PDF extends FPDF
{
    ...
    function agregarBOL($idPedido, $enlace) {
        ...
        // Agregar nueva página para cada sample después del primero
        ...
        $sqlEncabezado = "SELECT 
            ...
            '' AS Inv,
            enc.Cliente AS Destino_ClienteFinal,
            enc.PurchaseOrder,
            enc.CantidadEstibas,
            enc.Observaciones,
            ...
            CONCAT(trans.Nombre, ' - ', IFNULL(trans.Direccion, '')) AS TransportadoraFinal,
        ...
        if ($alturaDestino < $alturaMax) {
            $this->Line($xInicio, $yInicio + $alturaMax, $xInicio + 98, $yInicio + $alturaMax);
            $this->Line($xInicio + 98, $yInicio, $xInicio + 98, $yInicio + $alturaMax);
        }
        if ($alturaTransp < $alturaMax) {
            $this->Line($xInicio + 100, $yInicio + $alturaMax, $xInicio + 198, $yInicio + $alturaMax);
            $this->Line($xInicio + 198, $yInicio, $xInicio + 198, $yInicio + $alturaMax);
        }
        ...
        $this->Cell(2, 3, '', 0, 0,  'L');
        $this->Cell(98, 3, '', 'LR', 1,  'L');
        $this->Cell(98, 3, 'Charlottesville VA, 22902', 'LR', 0,  'L');
        $this->Cell(2, 3, '', 0, 0,  'L');
        $this->Cell(98, 3, '', 'LR', 1,  'L');
...
if ($pedidosProcesados === 0) {
    die(json_encode(["error" => "No se pudieron procesar los BOLs de samples"]));
}

// Generar nombre del archivo
$nombreArchivo = 'sample_bol_multiple_' . date('Y-m-d') . '.pdf';
