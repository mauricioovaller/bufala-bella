<?php
//src/Api/PedidosSample/ApiImprimirMultiplesListaEmpaque.php
...
$sqlPedidos .= " ORDER BY eps.FechaSalida, eps.Id_EncabPedido";
...
class PDF extends FPDF
{
    ...
    function agregarListaEmpaque($idPedido, $enlace) {
        ...
        $sqlEncabezado = "SELECT 
                            CONCAT('SAMPLE - ', eps.Id_EncabPedido) AS NoListaEmpaque,
                            DATE_FORMAT(eps.FechaOrden, '%W, %e de %M de %Y') AS FechaOrden,
                            DATE_FORMAT(eps.FechaSalida, '%W, %e de %M de %Y') AS FechaSalida,
                            eps.FechaEnroute,
                            eps.FechaDelivery,
                            DATE_FORMAT(eps.FechaDelivery, '%W, %e de %M de %Y') AS FechaEntregaCliente,
                            DATE_FORMAT(DATE_ADD(eps.FechaIngreso, INTERVAL 30 DAY), '%W, %e de %M de %Y') AS FechaVencimiento,
                            eps.GuiaMaster AS NoGuia,
                            '' AS FEX,
                            COALESCE(age.NOMAGENCIA, '') AS AgenciaCarga,
                            COALESCE(aer.NOMAEROLINEA, '') AS Aerolinea,
                            'GT Brokers Corp' AS Brokers,
                            '' AS Inv,
                            CONCAT(eps.Cliente, ' - ') AS Destino_ClienteFinal,
                            eps.PurchaseOrder,
                            eps.CantidadEstibas,
                            eps.Observaciones,
                            eps.Cliente AS NombreCliente,
                            '' AS Region,
                            '' AS Direccion,
                            trans.Nombre AS Transportadora,
                            bod.Descripcion AS Bodega
                          FROM EncabPedidoSample eps
                          LEFT JOIN Transportadoras trans ON eps.Id_Transportadora = trans.Id_Transportadora
                          LEFT JOIN Bodegas bod ON eps.Id_Bodega = bod.Id_Bodega
                          LEFT JOIN Agencias age ON eps.IdAgencia = age.IdAgencia
                          LEFT JOIN Aerolineas aer ON eps.IdAerolinea = aer.IdAerolinea
                          WHERE eps.Id_EncabPedido = ?";
        ...
        $sqlDetalle = "SELECT 
                       ...
                       INNER JOIN EncabPedidoSample eps ON det.Id_EncabPedido = eps.Id_EncabPedido
