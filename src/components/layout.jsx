...
import { Home, Users, ShoppingCart, Package, Menu, X, LogOut } from "lucide-react";
...
export default function Layout() {
  ...
  return (
    <>
      {/* Barra superior - NUEVO DISEÑO PROFESIONAL */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-slate-800 to-slate-900 text-white shadow-lg border-b border-slate-700 px-4 sm:px-6 py-3 flex justify-between items-center">
        ...
        {/* Menú en pantallas medianas/grandes - REDISEÑADO */}
      ...
      {/* Menú desplegable en móviles - REDISEÑADO COMPLETAMENTE */}
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-40 pt-16">
          ...
          <nav className="absolute top-0 right-0 w-80 h-full bg-slate-800 border-l border-slate-700 shadow-2xl">
            ...
            <div className="p-4 space-y-2">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-4 p-4 rounded-xl text-base font-medium transition-all duration-200 ${
                      isActive
                  ...
                  onClick={() => setMenuOpen(false)}
                >
                  <div className={`p-2 rounded-lg ${menuOpen ? 'bg-white/10' : ''}`}>
                    {item.icon}
                  </div>
                  <span>{item.label}</span>
                </NavLink>
            ...
            {/* Botón Salir en móviles - DESTACADO */}
      ...
      {/* Espacio para el contenido principal - MEJORADO */}
