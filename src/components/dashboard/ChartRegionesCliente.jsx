//src/components/dashboard/ChartRegionesCliente.jsx
...
const ChartRegionesCliente = ({ data, clienteNombre, color }) => {
    ...
    if (!data || data.length === 0) {
        return (
            <div className="text-center py-8 text-gray-500 border-2 border-dashed rounded-lg bg-gray-50">
                <p>No hay ventas por región para este cliente en el período seleccionado.</p>
            </div>
        );
    }
    ...
    const datosGrafico = data.map(item => ({
        region: item.region,
        regionCorta: item.region.length > (windowWidth < 768 ? 15 : 25)
            ? item.region.substring(0, windowWidth < 768 ? 12 : 22) + '...'
            : item.region,
    ...
    const maxLength = Math.max(...datosGrafico.map(item => item.regionCorta.length));
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="bg-white p-2 border border-gray-200 rounded-lg shadow-lg max-w-[250px]">
                    <p className="font-semibold text-gray-800 mb-1 text-sm break-words">{data.region}</p>
    ...
    return (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <h4 className="text-base font-semibold text-gray-700 mb-3">
                {/* Ventas por región: {clienteNombre} */}
            </h4>
            <div className="h-[250px] md:h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <YAxis type="category" dataKey="regionCorta" tick={{ fontSize: esMovil ? 10 : 11, fill: '#4B5563' }} width={leftMargin} axisLine={false} tickLine={false} tickMargin={esMovil ? 4 : 8} />
...
export default ChartRegionesCliente;