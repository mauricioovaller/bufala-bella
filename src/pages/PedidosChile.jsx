...
import PedidoHeader from "../components/pedidosChile/PedidoChileHeader";
import PedidoDetail from "../components/pedidosChile/PedidoChileDetail";
...
import {
  ...
  anularPedido,
  imprimirPedidoChile,
  imprimirPedidosChileMultiples,
  getRangoPedidosChile,
  contarPedidosChilePorFiltro,
...
export default function Pedidos() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (header.id && header.id !== 0) {
        ...
        if (res.success) {
          const nuevoNumero = `CHI-${String(res.idPedido).padStart(6, "0")}`;
  ...
  // 🔴 Búsqueda server-side: solo trae los pedidos Chile que coinciden con el término
  ...
  async function handleSelectPedido(id) {
    try {
      ...
      const mappedHeader = {
        id: apiHeader.Id_EncabPedidoChile ?? apiHeader.Id_EncabPedido ?? apiHeader.IdEncabPedido ?? apiHeader.id ?? null,
        numero: (apiHeader.Id_EncabPedidoChile ?? apiHeader.Id_EncabPedido ?? apiHeader.IdEncabPedido)
          ? `CHI-${String(apiHeader.Id_EncabPedidoChile ?? apiHeader.Id_EncabPedido ?? apiHeader.IdEncabPedido).padStart(6, "0")}`
          : header.numero,
      ...
      const mappedItems = (apiDetalle || []).map((d) => {
        ...
        return {
          id: d.Id_DetPedidoChile ?? d.Id_DetPedido ?? d.IdDetPedido ?? d.id ?? null,
          detId: d.Id_DetPedidoChile ?? d.Id_DetPedido ?? d.IdDetPedido ?? d.id ?? null,
  ...
  const handlePrint = async (tipoDocumento = null) => {
    ...
    try {
      ...
      const blob = await imprimirPedidoChile(header.id, tipoDocumento);
  ...
  const handlePrintMultiple = async (filtros) => {
    try {
      ...
      if (esPorFechas && esModoUnSolo) {
        ...
        const blob = await imprimirPedidosChileMultiples(filtros);
        const fileURL = URL.createObjectURL(blob);
        ...
        try {
          const resultado = await getRangoPedidosChile({
            modo: 'porFechas',
        ...
        Swal.fire({
          ...
          text: `Generando pedidos CHI-${filtros.numeroDesde} a CHI-${filtros.numeroHasta}`,
          allowOutsideClick: false,
          didOpen: () => {
            Swal.showLoading();
          }
        ...
        const blob = await imprimirPedidosChileMultiples(datosEnvio);
        const fileURL = URL.createObjectURL(blob);
        ...
        const confirmacion = await Swal.fire({
          ...
          <div class="text-left">
            ...
            <div class="mt-3 border-t pt-2">
              <p class="text-xs font-semibold">Filtros aplicados:</p>
              <p class="text-xs">• CHI-${filtros.numeroDesde} a CHI-${filtros.numeroHasta}</p>
              ${filtros.bodegaId ? `<p class="text-xs">• Bodega: ${filtros.bodegaId}</p>` : ''}
              <p class="text-xs">• Tipo: ${filtros.tipoDocumento}</p>
              <p class="text-xs">• Formato: Documentos individuales</p>
            </div>
      ...
      try {
        if (document.getElementById('archivo-actual')) {
          document.getElementById('archivo-actual').textContent =
            `Descargando: CHI-${pedido.id} - ${pedido.cliente ? pedido.cliente.substring(0, 30) : 'Sin cliente'}`;
        }

        const blob = await imprimirPedidoChile(pedido.id, tipoDocumento);
        ...
        console.error(`Error descargando CHI-${pedido.id}:`, error);
        errores.push(`CHI-${pedido.id}: ${error.message.substring(0, 50)}`);
      }
    };
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-white rounded-xl shadow-md p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-semibold text-slate-700">Gesti&oacute;n de Pedidos Chile</h2>
          ...
          <button
            onClick={() => Swal.fire({
              title: "Ayuda - Pedidos Chile",
              html: `
                <div style="text-align:left;font-size:14px;line-height:1.6;">
                  <p><strong>📝 ¿Cómo crear un pedido?</strong></p>
                  <ol style="padding-left:20px;margin-bottom:12px;">
                    <li>Complete los datos del encabezado: Cliente, Fechas, Aerolínea, etc.</li>
                    <li>Agregue productos al detalle con <strong>"+ Agregar Producto"</strong></li>
                    <li>Verifique que cada producto tenga descripción, embalaje, cantidad y precio</li>
                    <li>Click <strong>"Guardar Pedido"</strong></li>
                  </ol>
                  <hr style="border:none;border-top:1px solid #e5e7eb;margin:12px 0;">
                  <p><strong>✏️ ¿Cómo modificar?</strong></p>
                  <ol style="padding-left:20px;margin-bottom:12px;">
                    <li>Click <strong>"Buscar Pedidos"</strong>, seleccione el pedido y <strong>"Cargar"</strong></li>
                    <li>Modifique los campos necesarios</li>
                    <li>Click <strong>"Actualizar Pedido"</strong></li>
                  </ol>
                  <hr style="border:none;border-top:1px solid #e5e7eb;margin:12px 0;">
                  <p><strong>👁️ Visor de Producción</strong></p>
                  <p>Cada fila del detalle tiene un botón <strong>👁️</strong> que muestra el lote y fechas asignadas desde el módulo <strong>Despachos</strong> (seleccionando tipo "Chile").</p>
                </div>
              `,
              confirmButtonText: "Entendido", confirmButtonColor: "#2563EB", width: 560,
            })}
            className="flex items-center gap-1 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 hover:text-gray-800 rounded-lg text-sm font-medium transition shrink-0"
            title="Ayuda del módulo"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M12 17h.01" />
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-gray-900 font-medium">CHI-{String(p.idPedido).padStart(6, "0")}</p>
                          </div>
      ...
      <ModalImpresionMultiple
        ...
        bodegas={[]}
        tipoOrden="chile"
        contarPedidosFn={contarPedidosChilePorFiltro}
      />
